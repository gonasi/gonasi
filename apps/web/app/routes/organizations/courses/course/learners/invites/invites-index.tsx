...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  if (!role || role === 'editor') {
    return redirectWithError(
      `/${organizationId}/courses/${courseId}/published`,
  ...
  return (
    <>
      {shouldShowVisibilityBanner && (
        <div className='px-0 pb-8 md:px-4'>
          ...
          <BannerCard
            ...
            variant='tip'
            cta={{
              to: `/${params.organizationId}/courses/${params.courseId}/overview/edit-details?redirectTo=/${params.organizationId}/courses/${params.courseId}/learners/invites`,
      ...
      <div className='flex items-center justify-between px-0 md:px-4'>
        ...
        <IconNavLink
          to={`/${params.organizationId}/courses/${params.courseId}/learners/invites/new-invite`}
          icon={Plus}
                    ...
                    <TableCell className='flex h-full items-center justify-end gap-4 py-4'>
                      {status === 'Pending' ? (
                        <>
                          <div>
                            <IconNavLink
                              to={`/${params.organizationId}/courses/${params.courseId}/learners/invites/resend/${invite.id}/${invite.token}`}
                              icon={RotateCcw}
                              size={20}
                            />
                          </div>
                          <div>
                            <IconNavLink
                              to={`/${params.organizationId}/courses/${params.courseId}/learners/invites/revoke/${invite.id}/${invite.token}`}
                              icon={X}
                              size={20}
                            />
                          </div>
