...
import type { Route } from './+types/edit-pricing-tier';
...
export function meta() {
  return [
    { title: 'Edit Pricing Tier • Gonasi' },
    {
      name: 'description',
      content:
        'Edit pricing tier for your course on Gonasi. Update pricing details, access permissions, and enhance your monetization strategy effectively.',
    },
  ];
}
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  if (!courseId || !coursePricingId) {
    return redirectToPricingPage('Invalid course or pricing tier');
  }

  const [pricingTier, pricingData, availableFrequencies] = await Promise.all([
    fetchCoursePricingTierById({ supabase, coursePricingId }),
  ...
  const frequencyOptions: FrequencyOption[] = addFrequencyOption(
    pricingTier.payment_frequency ?? 'monthly',
...
export default function EditPricingTier({ params, loaderData }: Route.ComponentProps) {
  const { organizationId, courseId, coursePricingId } = params;
  const { pricingTier, isPaid, frequencyOptions } = loaderData;
  ...
  return (
    <Modal open>
      <Modal.Content size='md'>
        <Modal.Header
          title={isPaid ? 'Edit Pricing Tier' : 'Edit Free Access Tier'}
              ...
              <Form method='POST' onSubmit={methods.handleSubmit}>
                ...
                <div className='flex w-full justify-end'>
                  {isLastStep ? (
                    ...
                    >
                      {isPaid ? 'Update Pricing Tier' : 'Update Access Tier'}
                    </Button>
                  ) : (
                    <div className='h-12' />
                  )}
