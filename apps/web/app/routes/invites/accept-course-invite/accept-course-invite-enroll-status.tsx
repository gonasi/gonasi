...
import type { Route } from './+types/accept-course-invite-enroll-status';
...
export default function AcceptCourseInviteEnrollStatus({
  params,
  loaderData,
      ...
      <Modal.Content size='sm'>
        ...
        <Modal.Body className='space-y-4 px-4 py-4'>
          ...
          <div className='text-muted-foreground space-y-2 pb-4 text-sm'>
            {isSuccessful ? (
              <p>
                Your payment has been confirmed and you're now enrolled in{' '}
                <strong>{courseName}</strong> via invitation. You can start learning right away!
              </p>
            ) : (
              <>
                <p>
                  We couldn't verify your payment for <strong>{courseName}</strong>. This might be
                  due to a network delay or an incomplete transaction.
                </p>
                <p>
