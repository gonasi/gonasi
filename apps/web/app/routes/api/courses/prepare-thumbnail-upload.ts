...
/**
 * Prepares a thumbnail upload by generating Cloudinary signature.
...
export async function action({ request }: Route.ActionArgs) {
  try {
    ...
    if (!courseId || !organizationId || !mimeType || !size) {
      ...
      return Response.json(
        {
          success: false,
          message: 'Missing required fields: courseId, organizationId, mimeType, size',
        },
        { status: 400 },
      );
    }
    ...
    // Pattern: /:organizationId/courses/:courseId/thumbnail/draft/thumbnail
    ...
    const uploadSignature = generateUploadSignature({
      ...
      tags: ['draft', 'thumbnail', courseId, organizationId],
      context: {
        organizationId,
        courseId,
      },
