...
/**
 * Prepares an organization profile photo upload by generating Cloudinary signature.
...
export async function action({ request }: Route.ActionArgs) {
  try {
    ...
    console.log('[prepare-profile-photo-upload] Request params:', {
      organizationId,
      mimeType,
      size,
    });
    ...
    if (!organizationId || !mimeType || !size) {
      console.error('[prepare-profile-photo-upload] Missing required fields:', {
    ...
    if (permError) {
      console.error('[prepare-profile-photo-upload] Permission check error:', permError);
      return Response.json(
        {
          success: false,
    ...
    if (!canEdit) {
      console.error('[prepare-profile-photo-upload] Permission denied for user');
      return Response.json(
        {
          success: false,
    ...
    const uploadSignature = generateUploadSignature({
      ...
      tags: ['profile', 'avatar', organizationId],
      context: {
        organizationId,
      },
    ...
    console.log('[prepare-profile-photo-upload] Signature generated successfully');
    return Response.json({
      success: true,
      data: {
        uploadSignature,
    ...
    console.error('[prepare-profile-photo-upload] Unexpected error:', error);
    return Response.json(
      {
        success: false,
