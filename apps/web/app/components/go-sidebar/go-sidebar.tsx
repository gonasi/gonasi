    ...
    <aside className='border-r-card sticky top-16 h-full min-h-screen flex-none border-r md:top-20'>
      <div className='mb-10 py-2 pr-0 md:py-8 md:pr-10'>
        ...
        {isGoSuOrGoAdmin(role) && (
          <SideLink to='/go-admin/lesson-types' name='GO - Lesson Types' icon={<BookOpenCheck />} />
        )}
        <SideLink to={`/dashboard/${companyId}`} name='Dashboard' icon={<LayoutDashboard />} />
        <SideLink
        ...
        {activeCompany?.staff_role === 'su' || activeCompany?.staff_role === 'admin' ? (
          <>
            ...
            <SideLink
              ...
              icon={<BookLock />}
            />
            <SideLink to={`/dashboard/${companyId}/revenue`} name='Revenue' icon={<Coins />} />
          </>
        ) : null}
      </div>
