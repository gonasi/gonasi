...
function RadioGroup({ className, error, ...props }: RadioGroupProps) {
  return (
    <RadioGroupPrimitive.Root
      ...
      className={cn('grid gap-3', className, error && 'border-destructive')}
      {...props}
    />
  );
}
...
function RadioGroupItem({ className, error, ...props }: RadioGroupItemProps) {
  return (
    <RadioGroupPrimitive.Item
      ...
      className={cn(
        'border-input text-primary focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:bg-input/30 ml-2 aspect-square size-4 shrink-0 rounded-full border shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50',
        error && 'border-destructive', // Conditionally applying error styles
        className,
      )}
      {...props}
    >
