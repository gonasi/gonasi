...
import type { TapToRevealBase } from '.';

import useModal from '~/components/go-editor/hooks/useModal';
import { InsertTapToRevealDialog } from '~/components/go-editor/plugins/GoMenuPlugin/RevealPlugins/TapToRevealPlugin';
...
import { TapToRevealCard } from '~/components/go-editor/ui/tap-to-reveal-card';
import { TapToRevealActions } from '~/components/go-editor/ui/tap-to-reveal-card/TapToRevealActions';
import { useStore } from '~/store';

interface TapToRevealComponentProps extends TapToRevealBase {
  nodeKey: string;
}

export default function TapToRevealComponent({
  nodeKey,
  frontSideState,
  backSideState,
  uuid,
}: TapToRevealComponentProps) {
  ...
  const onEditNode = useCallback(() => {
    showModal(
      'Edit tap to reveal',
      (onClose) => (
        <InsertTapToRevealDialog
          activeEditor={editor}
          onClose={onClose}
          defaultData={{
            frontSideState: JSON.stringify(frontSideState),
            backSideState: JSON.stringify(backSideState),
            uuid,
          }}
          mode='edit'
          nodeKey={nodeKey}
  ...
  }, [editor, showModal, frontSideState, backSideState, uuid, nodeKey]);
  ...
  return (
    ...
    >
      ...
      >
        <TapToRevealCard
          front={<RichTextRenderer editorState={JSON.stringify(frontSideState)} />}
          back={<RichTextRenderer editorState={JSON.stringify(backSideState)} />}
          revealed={isPlayed}
          onToggle={setIsPlayed}
        />
      </QuizCard>
