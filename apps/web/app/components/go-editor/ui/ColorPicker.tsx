...
export default function ColorPicker({ color, onChange }: Readonly<ColorPickerProps>): JSX.Element {
  ...
  return (
    <div className='color-picker-wrapper' style={{ width: WIDTH }} ref={innerDivRef}>
      <input placeholder='Hex' onChange={(e) => onSetHex(e.target.value)} value={inputColor} />
    ...
    <div
      ...
      onKeyDown={(e) => {
        if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
          const x = e.key === 'ArrowLeft' ? 0 : WIDTH;
          onChange({ x, y: 0 });
        }
        if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
          const y = e.key === 'ArrowUp' ? 0 : HEIGHT;
          onChange({ x: 0, y });
        }
      }}
    >
