...
import { INSERT_MODEL3D_COMMAND, type InsertModel3DPayload } from '.';
...
export default function InsertModel3DDialog({
  activeEditor,
  onClose,
}: {
  ...
  useEffect(() => {
    ...
    searchUrl.searchParams.set('fileType', FileType.MODEL_3D);
  ...
  const handleModel3DInsert = (file: SearchFileResult) => {
    const payload: InsertModel3DPayload = {
      fileId: file.id,
    };

    activeEditor.dispatchCommand(INSERT_MODEL3D_COMMAND, payload);
    onClose();
  };
  ...
  return (
    <div className='pb-8'>
      ...
      >
        <SearchInput placeholder='Search for 3D model files...' />
      </motion.div>
      {isInitialLoading ? (
        <Spinner />
            ...
            <AnimatePresence>
              {fetcher.data.data.map((file) => (
                ...
                >
                  <PlainButton
                    key={file.id}
                    onClick={() => handleModel3DInsert(file)}
        ...
        <div className='text-muted-foreground mt-4 text-center'>
          This course doesn't have 3D model files yet. Add some to enhance your content.
        </div>
      )}
    </div>
  );
