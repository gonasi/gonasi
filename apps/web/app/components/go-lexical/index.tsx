...
export default function GoLexical({
  ...
  return (
    <LexicalComposer initialConfig={editorConfig}>
      <div
        className={cn(
          'relative mx-auto mt-5 mb-5 max-w-[600px] rounded-t-[10px] text-left leading-5 font-normal text-black',
        )}
      >
        <ToolbarPlugin />
        <div className={cn('relative bg-white')}>
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                className={cn(
                  'tab-[1] relative min-h-[150px] resize-none px-[10px] py-[15px] text-[15px] caret-[#444] outline-none',
                )}
                aria-placeholder={placeholder}
                placeholder={
                  <div
                    className={cn(
                      'pointer-events-none absolute top-[15px] left-[10px] inline-block overflow-hidden text-[15px] text-ellipsis text-[#999] select-none',
                    )}
                  >
                    {placeholder}
                  </div>
