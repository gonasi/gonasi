 ...
 */
export async function setCourseFree({
  supabase,
  courseId,
}: {
  ...
  const { error } = await supabase.rpc('set_course_free', {
    p_course_id: courseId,
    p_user_id: userId,
  });

  if (error) {
    console.error('Failed to mark course as free:', error.message);
    return {
      success: false,
      message: error.message || 'Unable to mark the course as free.',
    };
  }

  return {
    success: true,
    message: 'Course successfully marked as free.',
  };
}
