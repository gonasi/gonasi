...
/**
 * Sets a live session to live mode for the current user.
 ...
 * @param sessionId - The ID of the session to switch to live mode
 ...
 */
export async function setLiveSessionToLive({
  supabase,
  sessionId,
  organizationId,
}: {
  ...
  if (permissionError || !canEdit) {
    console.error(
      '[setLiveSessionToLive] Permission denied to update session mode:',
  ...
  // Update the mode to live - the trigger will handle state reset
  ...
  if (error) {
    console.error('[setLiveSessionToLive] Failed to set session to live mode:', error.message);
    return {
      success: false,
      message: error.message || 'Unable to switch to live mode.',
    };
  }

  return {
    success: true,
    message: 'Session switched to live mode successfully. All session data has been reset.',
  };
}
