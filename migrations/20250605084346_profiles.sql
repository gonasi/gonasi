...
-- RLS Policy: Anyone (including anonymous) can read profile data
...
-- RLS Policy: Only allow a user to create their own profile
...
-- RLS Policy: Only allow a user to update their own profile
...
-- RLS Policy: Only allow a user to delete their own profile
...
-- ===================================================
-- AVATARS BUCKET SETUP (storage.objects)
-- ===================================================

-- Create a public S3-like bucket named "avatars" (if not exists)
insert into storage.buckets (id, name, public)
values ('avatars', 'avatars', true)
on conflict (id) do nothing;

-- RLS Policy: Allow public read access to avatar images
create policy "Allow public access to avatar images"
on storage.objects
for select
using (bucket_id = 'avatars');

-- RLS Policy: Anyone can upload avatar images
create policy "Allow uploading avatar images"
on storage.objects
for insert
with check (bucket_id = 'avatars');

-- RLS Policy: Only the owner of the avatar can update it
...
-- RLS Policy: Only the owner of the avatar can delete it
create policy "Allow user to delete own avatar"
on storage.objects
for delete
using ((select auth.uid()) = owner and bucket_id = 'avatars');

-- Support logical replication (for Supabase Realtime)
alter table public.profiles replica identity full; 

-- Enable real-time updates on the profiles table
