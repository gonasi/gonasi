...
CREATE TRIGGER trg_update_total_enrollments AFTER INSERT ON public.course_enrollment_activities FOR EACH ROW EXECUTE FUNCTION public.update_total_enrollments_on_activity();

CREATE TRIGGER trg_update_active_enrollments_insert AFTER INSERT ON public.course_enrollments FOR EACH ROW EXECUTE FUNCTION public.update_active_enrollments_count();

CREATE TRIGGER trg_update_active_enrollments_update AFTER UPDATE ON public.course_enrollments FOR EACH ROW WHEN (((old.is_active IS DISTINCT FROM new.is_active) OR (old.expires_at IS DISTINCT FROM new.expires_at))) EXECUTE FUNCTION public.update_active_enrollments_count();
