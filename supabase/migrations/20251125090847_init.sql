...
CREATE OR REPLACE FUNCTION public.check_member_limit_for_org(p_organization_id uuid, p_check_type text DEFAULT 'invite'::text)
 RETURNS TABLE(exceeded boolean, allowed integer, current integer, remaining integer, active_members integer, pending_invites integer, check_type text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.cleanup_file_variants()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.get_org_notifications_for_member(p_organization_id uuid, p_user_id uuid, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, organization_id uuid, key public.org_notification_key, category public.org_notification_category, title text, body text, link text, payload jsonb, created_at timestamp with time zone, read_at timestamp with time zone, dismissed_at timestamp with time zone, is_read boolean, is_dismissed boolean, visibility jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
CREATE TRIGGER trigger_cleanup_file_variants AFTER DELETE ON public.file_library FOR EACH ROW EXECUTE FUNCTION public.cleanup_file_variants();
